...
const LocationFlowDiagram: React.FC<LocationFlowDiagramProps> = ({ data, storyId, targetLocationId }) => {
    const theme = useTheme();
    ...
    const handleToggle = useCallback(async (locationId: number, isExpanded: boolean) => {
        if (isExpanded) {
            const updatedNodes = nodes.map(n => 
                n.id === locationId.toString() ? { ...n, data: { ...n.data, isExpanded: false } } : n
            );
            ...
            updatedNodes.forEach(n => {
                if (n.data.isInitial || n.id === targetLocationId.toString()) {
                    legitNodes.add(n.id);
                    queue.push(n.id);
                }
            });
        ...
        try {
            setLoading(true);
            const flow = await fetchLocationFlow([locationId]);
            ...
            const updatedNodes = nodes.map(n => 
                n.id === locationId.toString() ? { ...n, data: { ...n.data, isExpanded: true } } : n
            );
            ...
            const newNodes = newNodesData.map(n => ({
                ...
                data: { 
                    ...n, 
                    storyId, 
                    isTarget: n.id === targetLocationId,
                    isExpanded: false,
                    isInitial: false,
                    onToggle: (id: number, exp: boolean) => handleToggleRef.current(id, exp),
                    onNavigate: (id: number) => navigate(`/world/${storyId}?category=Locations&id=${id}`)
                },
                position: { x: 0, y: 0 }
            }));
            ...
            const newEdges = newEdgesData.map(e => ({
                id: `e-${e.id}`,
                source: e.sourceLocationId.toString(),
                target: e.targetLocationId.toString(),
                label: e.label,
                animated: true,
                type: 'smoothstep',
            ...
            console.error('Failed to expand location flow', err);
        } finally {
            setLoading(false);
        }
    }, [nodes, edges, targetLocationId, setNodes, updateGraphLayout, storyId, navigate, theme.palette.text.primary, theme.palette.background.paper, theme.palette.primary.main]);
        ...
        if (data) {
            const initialNodes: Node[] = data.nodes.map((s) => ({
                ...
                data: { 
                    ...s, 
                    storyId, 
                    isTarget: s.id === targetLocationId,
                    isExpanded: false,
                    isInitial: true,
                    onToggle: (id: number, exp: boolean) => handleToggleRef.current(id, exp),
                    onNavigate: (id: number) => navigate(`/world/${storyId}?category=Locations&id=${id}`)
    ...
    }, [data, storyId, targetLocationId, updateGraphLayout, navigate, theme]);
